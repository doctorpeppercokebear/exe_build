...
class FirmwareFlasher:
    ...
    def setup_ui(self):
        ...
        # 타이틀
        title_label = ttk.Label(main_frame, text="ESP32-S3 펌웨어 업로드", 
                                font=('Arial', 16, 'bold'))
        ...
        refresh_btn = ttk.Button(main_frame, text="새로고침", command=self.refresh_ports)
        ...
        baud_combo = ttk.Combobox(main_frame, textvariable=self.baud_var, 
        ...
        files_info = [
            ("Bootloader:", self.bootloader_path, "0x1000"),
        ...
        for idx, (label, path, addr) in enumerate(files_info):
            ...
            status_label = ttk.Label(info_frame, text=status, foreground=color, 
                                    font=('Arial', 10, 'bold'))
            status_label.grid(row=idx, column=1, padx=5)
            
            ttk.Label(info_frame, text=f"{addr} - {os.path.basename(path)}", 
                     font=('Arial', 9)).grid(row=idx, column=2, sticky=tk.W, padx=5)
        ...
        self.flash_btn = ttk.Button(btn_frame, text="펌웨어 업로드 시작", 
                                    command=self.start_flashing, width=25)
        self.flash_btn.grid(row=0, column=0, padx=5)
        
        self.clear_btn = ttk.Button(btn_frame, text="로그 지우기", 
